...
class _MySearchState extends State<MySearch> {
  ...
  Widget build(BuildContext context) {
    return StreamProvider.value(
        ...
        initialData: null,
        child: MySearchData(user: widget.user,)
    );
  }
}
...
class _MySearchDataState extends State<MySearchData> {
  ...
  bool vis=false;List<Journey> searchlist=[];
  @override
  Widget build(BuildContext context) {
          ...
          children: [
                    ...
                    icon: IconButton(
                      onPressed: () {
                        ...
                        showSearch(
                            ...
                            delegate: CustomSearchDelegate(searchcont: endLoc,searchTerms: endlocat)
                        );
            ...
            Visibility(
                ...
                child: Container(height: MediaQuery.of(context).size.height/10,
                  child: ListView.builder(itemCount: searchlist.length,itemBuilder: (BuildContext context, int index) {
                    return GestureDetector(
                      child: Container(padding: EdgeInsets.only(top: 10,right: 20,bottom: 10),
                        height: MediaQuery.of(context).size.height/25,
                        child: Card(child : Column(
                              children: [
                                Row(mainAxisAlignment: MainAxisAlignment.spaceEvenly,children: [
                                  Expanded(flex:1,child:Text('journey'),),
                                  Expanded(flex: 2,child: Text(searchlist[index].startingloc)),
                                  Expanded(flex:1,child:Text('------->'),),
                                  Expanded(flex:2,child: Text(searchlist[index].endingloc))
                                ],),
                                Row(mainAxisAlignment: MainAxisAlignment.spaceEvenly,children: [
                                  Expanded(flex:1,child:Text('Time'),),
                                  Expanded(flex: 2,child: Text(searchlist[index].startingtime)),
                                  Expanded(flex:1,child:Text('-----to------'),),
                                  Expanded(flex:2,child: Text(searchlist[index].endingtime))
                                ],),
                                Row(
                                  children: [
                                    Expanded(flex:1,child:Text('Remaining setas'),),
                                    Expanded(flex: 1,child: Text(searchlist[index].remseats)),
                                  ],
                                ),Row(
                                  children: [
                                    Expanded(flex:1,child:Text('journey driver'),),
                                    Expanded(flex: 1,child: Text(searchlist[index].email)),
                                  ],
                                ),
                          ],
                        )),
                      ...
                      onTap: (){Navigator.push(context,
                          MaterialPageRoute(builder: (context)=>SelectJourney(
                            user: widget.user,selected: searchlist[index],)));},
                    );
                  },
                  ),
                )),
  ...
  List<Journey> checker(journeylist){
    ...
    for(int i=0;i<journeylist.length;i++){
      ...
      if(journeylist[i].startingloc ==startLoc.text
          && journeylist[i].endingloc==endLoc.text && double.parse(journeylist[i].remseats)>0){
    ...
    return Scaffold(
      ...
      body: Column(
        children: [Text(widget.selected.email),
          ...
          Row(
            children: [
              Expanded(flex:1,child:Text('journey begins: '),),
              Expanded(flex: 1,child: Text(widget.selected.startingtime)),
            ],
          ),
          Row(
            children: [
              Expanded(flex:1,child:Text('journey ends: '),),
                  ...
                  child: TextField(readOnly: true,
                    ...
                    decoration: InputDecoration(
                      ...
                      icon: IconButton(onPressed: ()async{
                        await showDialog(context: context, builder: (_){
                          return Dialog(
                            child: OpenStreetMapLink(user: widget.user,loctra: startingloc,lat: lat,long: long),
                          );
                  ...
                  child: TextField(readOnly: true,
                    ...
                    decoration: InputDecoration(
                      ...
                      icon: IconButton(onPressed: ()async{
                        await showDialog(context: context, builder: (_){
                          return Dialog(
                            child: OpenStreetMapLink(user: widget.user,loctra: endingloc,lat: lat,long: long),
                          );
        ...
        await JourneyDriverTravellerConnection(
            driverid: widget.selected.email).AddAdminJourneyUserData(widget.selected.date,
                        ...
                        numseats.text,widget.selected.journeyid,startlat,startlong,endlat,endlong);
        await JourneyDriverTravellerConnection(
                driverid: widget.selected.email).UpdateDriverJourneyDataInUser(remseats.toString(),
                        widget.selected.journeyid);
        await clear();
    ...
    return ListView.builder(
      ...
      itemBuilder: (context, index) {
        ...
        return GestureDetector(
          child: ListTile(
            title: Text(result),
          ),onTap: (){searchcont.text=result.toString();
        close(context, null);
            },
        );
      },
